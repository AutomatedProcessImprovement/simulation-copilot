...
from .batching import BatchingRule
from .case_attributes import CaseAttribute
from .extraneous_delays import ExtraneousDelay
from .prioritization import PrioritizationRule
...
class BPSModel:
    ...
    def to_prosimos_format(self) -> dict:
        ...
        if self.case_attributes is not None:
            attributes[CASE_ATTRIBUTES_KEY] = [
                case_attribute.to_prosimos() for case_attribute in self.case_attributes
            ]
        ...
        attributes["granule_size"] = {
            "value": self.calendar_granularity,
            "time_unit": "MINUTES",
        }
    ...
    def replace_activity_names_with_ids(self):
        ...
        if self.resource_model.activity_resource_distributions is not None:
            ...
            ) in self.resource_model.activity_resource_distributions:
    ...
    def to_json(self, output_dir: Path, process_name: str) -> Path:
        json_parameters_path = output_dir / f"{process_name}.json"
